...
export class ProductRepository{
    ...
    updateProduct(id:number, name: string, price: number) {
        try {
            const resultado = executarComandoSQL('UPDATE vendas.Product SET name = ?, price = ? WHERE id = ?', [name, price, id], this.imprimeResult);
    ...
    deleteProduct(id:number) {
        try {
            const resultado = executarComandoSQL('DELETE FROM vendas.Product WHERE id = ?', [id], this.imprimeResult);
